...
**Planned deviation for reproduction**: Here, we vary from the original study design by changing the population threshold within the SpatialEpi-Kulldorff Spatial Scan from 50% to 10%. 
This should produce more localized COVID-19 clustering results. 

```{r New SpatialEpi-Kulldorff, eval = FALSE, fig.width=4, fig.height=4}
start_time <- Sys.time()
covid_geo <- covid_table %>%
  select(x, y) %>%
  latlong2grid()
...
covid_kulldorff <- kulldorff(
  ...
  population = covid_table$pop,
  expected.cases = expected.cases,
  pop.upper.bound = 0.1,                     # Change from 50% of counties to 10%.
  n.simulations = 999,
  alpha.level = 0.05,
  plot = TRUE
)
...
```{r mapFuctionJ}
mapData <- mapData(data, title, style, palette) {
  stylizedMap <- 
    tm_polygons()
}
