...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Tate Sutter"
date: "`r Sys.Date()`"
...
Version 2.1 \| First Created July 7, 2021 \| Updated October 4, 2023
...
missingData <- acs_covid %>% filter(is.na(bpov_pct)) %>% st_drop_geometry() 
missingData <- t(missingData)

missingData %>% 
  kable(
    col.names = "",
    caption = "Table of County Missing Disability and Povert Data",
    aling = "c"
  ) %>% 
...
This section is being skipped, so the research design can be readjusted.
The upper population bounding is changed from 50% to 10% of the total population.
Ideally, this will produce smaller, more region specific clusters.
...
**Planned deviation for reproduction**: Here, we vary from the original study design by changing the upper population bounding limit for cluster creation within the SpatialEpi-Kulldorff Spatial Scan from 50% to 10%.
This should produce more localized COVID-19 clustering results.

```{r New SpatialEpi-Kulldorff, eval = TRUE, fig.width=4, fig.height=4}
start_time <- Sys.time()
covid_geo <- covid_table %>%
  select(x, y) %>%
  latlong2grid()
...
covid_kulldorff <- kulldorff(
  ...
  population = covid_table$pop,
  expected.cases = expected.cases,
  pop.upper.bound = 0.1,                     # Change from 50% of population to 10%.
  n.simulations = 999,
  alpha.level = 0.05,
  plot = TRUE
)
...
tm_spatialepi_clusters <-
  ...
  tm_polygons(col = "isCluster",
          ...
          colorNA = NULL,
          title = "SpatialEpi Kulldorff COVID-19 Clusters with 10% Threshold",
          border.col = "white",
          lwd = 0.2,
          border.alpha = 0.2) +
  tm_shape(state) +
...
Between the 10% and 50% population threshold / upper population bounds renditions of the Kulldorff Spatial Scan, the 10% Scan produces a more compact cluster. The smaller threshold directs the Spatial Scan to limit its clustering at 10% of the total contiguous states' population. Considering decreased populations; therefore, fewer counties will produce more localized clusters. Smaller clusters may provide a more targeted result of impacted counties. With more localized clusters, resources can be more efficiently distributed. 
...
We adjusted the upper population bounding threshold in the Kuldorff Spatial Scan from 50% of the entire population to 10% of the entire population.
For much of the country, the Scan produced similar results.
Notably, in the Southeast results differed.
The 50% threshold produced a cluster centered in Alabama that stretched from Western Louisiana to South Carolina and from most of Tennessee to Southern Florida.
The 10% threshold decreased the size of the Southeast cluster to include most of Mississippi, Louisiana, and Alabama.
Smaller clusters emerge in Georgia and Florida.

The finer scale of the new clusters grant a greater understanding of COVID-19 clusters. 
With the Southeast divided into numerous clusters, regional variation of COVID-19 impacts become apparent.
Identifying a region as a cluster is useful since it illustrates that the the region as a whole faces greater challenges than other locations.
The finer threshold parses the variation within the coarse threshold's broad results. 
At the 10% threshold, the Southeast has a higher frequency of clusters than other regions of the United States.
The high prevalence of COVID-19 in the Southeast still appears; however, the clusters' close proximity enable the most impacted communities to be better identified.
